...
class CurrencyService:
    def __init__(
        self, repository: CurrencyRepository, providers: list[ExchangeRateProvider]
    ):
    ...
    async def initialize_supported_currencies(self) -> None:
        ...
        provider_tasks = [
            provider.fetch_supported_currencies() for provider in self.providers
        ]
        ...
        currency_models = [
            SupportedCurrency(code=code, name=None) for code in supported_codes
        ]
